...
for (const rule of redirects) {
  ...
  if (![301, 302, 303, 307, 308, 200].includes(rule.code)) {
    redirectIssues.push(`Invalid redirect code for "${rule.from}": ${rule.code}`);
  }

  if (rule.to.startsWith('/')) {
    const targetPath = rule.to === '/' ? 'index.html' : rule.to.replace(/^\//, '');
    if (!fs.existsSync(path.join(ROOT, targetPath))) {
      redirectIssues.push(`Redirect target does not exist for "${rule.from}": ${rule.to}`);
    }
  }
}
...
for (const htmlFile of htmlFiles) {
  ...
  for (const navBlock of navBlocks) {
    const linkMatches = navBlock.match(/href\s*=\s*(?:"([^"]+)"|'([^']+)'|([^\s>]+))/gi) || [];
...
console.log(`✅ Navigation and redirect simulation passed (${htmlFiles.length} pages, ${navLinksChecked} nav links, ${redirects.length} redirect rules).`);
