...
function isDecorativeImage(img) {
  const ariaHidden =
    (img.getAttribute("aria-hidden") || "").toLowerCase() === "true";
  const role = (img.getAttribute("role") || "").toLowerCase();
  const decorativeFlag =
    (img.getAttribute("data-decorative") || "").toLowerCase() === "true";
...
export async function strictVerifyDist({ distDir, site }) {
  ...
  assert(
    /^\d+$/.test(expectedLicense),
    `license_number must be digits only; got: ${expectedLicense}`,
  );

  assert(await exists(path.join(distDir, "_headers")), "Missing dist/_headers");
  assert(
    await exists(path.join(distDir, "_redirects")),
    "Missing dist/_redirects",
  );
  assert(
    await exists(path.join(distDir, "sitemap.xml")),
    "Missing dist/sitemap.xml",
  );
  assert(
    await exists(path.join(distDir, "robots.txt")),
    "Missing dist/robots.txt",
  );
  ...
  for (const file of indexFiles) {
    ...
    for (const s of site.validation?.forbidden_strings || []) {
      if (s && html.includes(s))
        throw new Error(`${file}: forbidden string found in output: "${s}"`);
    }
    ...
    assert(
      ...
      `${file}: canonical mismatch: "${canonical}" expected "${expectedCanonical}"`,
    );
    assert(
      (
        root.querySelector('meta[name="robots"]')?.getAttribute("content") || ""
      ).trim().length > 0,
      `${file}: missing meta robots`,
    );
    assert(
      root.querySelectorAll("h1").length === 1,
      `${file}: must contain exactly ONE <h1>`,
    );
    assert(
      !!root.querySelector('a.skip-link[href="#main"]'),
      `${file}: missing skip link to #main`,
    );
    assert(
      !!root.querySelector("main#main"),
      `${file}: missing <main id="main">`,
    );

    const m = root.text.match(
      new RegExp(`${site.license_label}\\s*#\\s*(\\d+)`, "i"),
    );
    assert(
      m && m[1] === expectedLicense,
      `${file}: license mismatch or missing`,
    );
    ...
    for (const link of root.querySelectorAll('link[rel="stylesheet"]')) {
      ...
      assert(
        await exists(path.join(distDir, href.replace(/^\//, ""))),
        `${file}: missing stylesheet asset ${href}`,
      );
    }
    for (const script of root.querySelectorAll("script")) {
      ...
      assert(
        await exists(path.join(distDir, src.replace(/^\//, ""))),
        `${file}: missing script asset ${src}`,
      );
    }
    ...
    for (const img of root.querySelectorAll("img")) {
      ...
      if (alt.length === 0)
        assert(
          ...
          `${file}: <img> alt="" but not marked decorative`,
        );
    }
    ...
    for (const input of root.querySelectorAll("input, textarea, select")) {
      if ((input.getAttribute("type") || "").toLowerCase() === "hidden")
        continue;
      assert(
        hasAccessibleLabel(root, input),
        `${file}: form control missing label/aria-label`,
      );
    }
    ...
    for (const a of root.querySelectorAll("a")) {
      ...
      if (/\.(css|js|png|jpg|jpeg|webp|avif|svg|pdf|ico)$/i.test(href)) {
        assert(
          await exists(path.join(distDir, href.replace(/^\//, ""))),
          `${file}: missing asset ${href}`,
        );
      } else {
        const pagePath =
          href === "/"
            ? path.join(distDir, "index.html")
            : path.join(distDir, href.replace(/^\//, ""), "index.html");
