...
const TARGET_BASENAMES = new Set(['.htaccess', '_redirects', 'nginx.conf', 'apache2.conf', 'httpd.conf']);
...
function walk(dir, out = []) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (TARGET_EXTENSIONS.has(ext) || TARGET_BASENAMES.has(entry.name)) out.push(full);
  }
  return out;
}
...
if (failures.length) {
  console.error('❌ Found origin-level redirect patterns that can conflict with Cloudflare edge redirects:\n');
...
console.log(`✅ No forbidden origin-level HTTPS/host redirect patterns found in ${files.length} config/source files.`);
