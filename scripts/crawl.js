...
const detectPrimaryCta = (html) => {
  const buttonRegex = /<(a|button)[^>]*class=["'][^"']*btn-primary[^"']*["'][^>]*>/i;
...
const detectPageType = (pathname) => {
  ...
  if (page.endsWith('testimonials.html') || page.endsWith('reviews.html')) return 'reviews';
...
const crawl = async () => {
  ...
  while (queue.length) {
    ...
    try {
      ...
      visited.set(current, { url: current, status: 'error', error: error.message });
      continue;
    }
  ...
  fs.writeFileSync(path.join(outputDir, 'crawl_raw.json'), JSON.stringify(report, null, 2));
  ...
  fs.writeFileSync(path.join(outputDir, 'BASELINE_URL_INVENTORY.csv'), rows.join('\n'));
};
