...
function validateServices(services) {
  for (const service of services) {
    if (service.quickAnswers.length < 2) {
      throw new Error(
        `Service "${service.slug}" has <2 quick answers. Add AEO Q&A.`,
      );
    }

    const combined = service.quickAnswers
      .map((item) => `${item.q} ${item.a}`)
      .join(" ");
    if (wordCount(combined) < 60) {
      throw new Error(
        `Service "${service.slug}" quick answers are too thin. Expand answers.`,
      );
    }
  }
}
...
function validatePages(corePages) {
  ...
  if (!slugs.has("/services"))
    throw new Error("Missing /services in CORE_PAGES");
  if (!slugs.has("/contact")) throw new Error("Missing /contact in CORE_PAGES");
}

function main() {
  const services = loadArrayExport(
    resolve("src/config/services.ts"),
    "SERVICES",
  );
  const corePages = loadArrayExport(
    resolve("src/config/pages.ts"),
    "CORE_PAGES",
  );
